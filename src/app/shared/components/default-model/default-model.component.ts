...
export class DefaultModelComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if(this.inputData.dataValues.selectedCustomer) {
  ...
  addNew(){
    if(!this.inputData.dataValues.updateJob){
      const data = {...this.inputData, createNew: true};
      this.dialog.close({data});
    } else{
      const data = {...this.inputData, job: 'update'};
      this.dialog.close({data});
    }
  ...
  close(){
    const data = {...this.inputData.dataValues.data, createNew: false};
    this.dialog.close({data});
  }
  ...
  autoCompleteEquipments(list){
    const newList = [];
    list.map(l => {
        newList.push(this.autoComplet(l._id, l.info.nfcTag));
    });
    return newList;  
  }
  ...
  cancelJob(){
    ...
    dialogRef.afterClosed().subscribe((data:boolean) =>{
      if(data){
        const data = {...this.inputData, job:'cancel'};
        this.dialog.close({data});
      } else{
        this.close();
      }
  ...
  uploadFile(container) {
    ...
    this.apiProvider.getImage(param).subscribe((imageData:any) => {
        ...
        if (imageData['status'] = '1') {
          ...
          const data = {...this.inputData.dataValues.data, createNew: true};
          this.dialog.close({data});
        }
    }, (err) => {
