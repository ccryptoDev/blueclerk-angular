...
import {
  ...
  navItemsTechManager,
  navItemsGym,
  navItemsManufacture /*, navMainItems*/,
} from "../../_nav";
...
export class RoleTechinicianListComponent {
  ...
  @ViewChild("confirmDeleteDlg", { static: false })
  confirmDeleteDlg: ModalComponent;
  ...
  constructor(
    ...
    if (this.userInfo.getUserInfo("type") == 1) {
      ...
      this.userInfo.getUserInfo("type") == 2 ||
      this.userInfo.getUserInfo("type") == 5
    ) {
  ...
  filterSearch() {
    ...
    this.techList.map((item) => {
      if (
        ...
        item.name.toLowerCase().indexOf(this.searchOpt.name.toLowerCase()) == -1
      ) {
        return;
      }
      list.push(item);
