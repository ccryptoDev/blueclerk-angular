...
export class AdminComponent implements OnInit {
  ...
  optionlistGroup: CdkDropListGroup<CdkDropList>;
  ...
  constructor(
    ...
    private dialog: MatDialog // end for drag
  ) {
    this.route.data.subscribe((res) => {
      if (res.openModal) this.onClickReportNumber();
      else {
  ...
  onCreateType() {
    this.router.navigate(["/admin/equipment-type"]);
  }
  ...
  onCreateJobType() {
    this.router.navigate(["/admin/job-types"]);
  }
  onConfirmJobType() {
  ...
  filterSearch() {
    ...
    this.allBrandList.map((item) => {
      if (
        this.searchOpt.brand.length > 0 &&
        item.name.toLowerCase().indexOf(this.searchOpt.brand.toLowerCase()) ==
          -1
      ) {
        return;
      }
  ...
  onClickReportNumber() {
    ...
    dialogRef.afterClosed().subscribe((data) => {
      if (data.data.createNew) {
        ...
        this.apiProvider
          .customWorkOrderNumber(body)
          .subscribe((response: any) => {
            if (response.status == 1) {
              let companyInfo = JSON.parse(
                this.userInfo.getUserData("company")
              );
  ...
  typefilterSearch() {
    ...
    this.allTypeList.map((item) => {
      if (
        this.searchOpt.type.length > 0 &&
        item.typename
          .toLowerCase()
          .indexOf(this.searchOpt.type.toLowerCase()) == -1
      ) {
        return;
      }
      list.push(item);
  ...
  jobfilterSerach() {
    ...
    this.items.map((item) => {
      if (
        this.searchOpt.jobtype.length > 0 &&
        item.name.toLowerCase().indexOf(this.searchOpt.jobtype.toLowerCase()) ==
          -1
      ) {
        return;
      }
  ...
  groupfilter() {
    ...
    this.groupList.map((item) => {
      if (
        this.searchOpt.groupname.length > 0 &&
        item.name
          .toLowerCase()
          .indexOf(this.searchOpt.groupname.toLowerCase()) == -1
      ) {
        return;
      }
