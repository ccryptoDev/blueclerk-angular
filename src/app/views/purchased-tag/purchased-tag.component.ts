...
export class PurchasedTagComponent implements OnInit {
  ...
  getPurchasedTags() {
    this.curTagCnt = 0;
    this.apiProvider.getTags().subscribe(
      (res: { status: number, orders: any }) => {
        if (res.status == 1) {
          this.tagList = res.orders;
          this.listTitle = `${res.orders.length || 0} Purchases`
          return;
        }
  ...
  goToInvoice(event) {
    // we will need to have pop up functionality to show the invoice details
