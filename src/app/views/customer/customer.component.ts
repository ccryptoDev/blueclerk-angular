...
export class CustomerComponent {
    ...
    this.apiProvider.getSyncInfo().subscribe(
      (res: {
        ...
        if (res.status === 1) {
          ...
          if (customerSyncData.customersSynced)
            customerSyncData.customersSyncedAt = res.customersSyncedAt;
  ...
  getCustomerDetails(customerId: string) {
    this.apiProvider.getCustomerDetail({ customerId }).subscribe((res) => {
  ...
  getCustomer(includeActive, includeNonActive) {
    ...
    this.serviceStatus =
      includeActive === "true" ? ServiceStatus.ACTIVE : ServiceStatus.IN_ACTIVE;
  }
  ...
  filterSearch() {
    ...
    try {
      this.customerdata.map((item) => {
        if (
          this.searchOpt.name.length > 0 &&
          item.info.name
            .toLowerCase()
            .indexOf(this.searchOpt.name.toLowerCase()) == -1
        ) {
          return;
        }
        list.push(item);
  ...
  Equiplist(item) {
    ...
    this.router.navigate(["/equipment"], {
      queryParams: { _id: customer_id, customer_name: customer_name },
  ...
  goToVisitList(item) {
    ...
    this.router.navigate(["/schedule"], {
      queryParams: { _id: customer_id, customer: customer },
      skipLocationChange: true,
    });
  }
