...
export class RatesComponent implements OnInit {
  ...
  editTheJob(event) {
    ...
    const editItemType = [editItem.jobType];
    let modelData = this.generateModelValues();
    const currentJobType = this.autoCompleteJob(editItemType);
    modelData.dataInputs.jobTypes.push(currentJobType[0]);
    ...
    modelData.dataValues.title = 'Edit Item';
    if (event.salesTax) {
    ...
    this.apiProvider.getJobCharges().subscribe((charges: any) => {
      ...
      this.apiProvider.getJobType().subscribe((jobTypes: any) => {
        ...
        if (jobTypes.status == 1) {
          console.log(this.jobTypes);
          this.allJobTypes = jobTypes.types;
          this.formatJobTypes(jobTypes.types);
        }
      });
    });
  }
  ...
  formatJobTypes(jobTypes) {
    jobTypes.map((type, i) => {
      let match = false;
      this.charges.map((charge: any) => {
        if (type._id === charge.jobType._id) {
          match = true;
        }
      });
      if (!match) {
